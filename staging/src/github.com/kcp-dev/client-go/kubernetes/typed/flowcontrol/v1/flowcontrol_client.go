...
type FlowcontrolV1ClusterClient struct {
	clientCache kcpclient.Cache[*flowcontrolv1.FlowcontrolV1Client]
}
...
// NewForConfig creates a new FlowcontrolV1ClusterClient for the given config.
...
// NewForConfigAndClient creates a new FlowcontrolV1ClusterClient for the given config and http client.
// Note the http client provided takes precedence over the configured transport values.
func NewForConfigAndClient(c *rest.Config, h *http.Client) (*FlowcontrolV1ClusterClient, error) {
	cache := kcpclient.NewCache(c, h, &kcpclient.Constructor[*flowcontrolv1.FlowcontrolV1Client]{
		NewForConfigAndClient: flowcontrolv1.NewForConfigAndClient,
	})
	if _, err := cache.Cluster(logicalcluster.Name("root").Path()); err != nil {
		return nil, err
	}

	return &FlowcontrolV1ClusterClient{clientCache: cache}, nil
}

// NewForConfigOrDie creates a new FlowcontrolV1ClusterClient for the given config and
...
func setConfigDefaults(config *rest.Config) {
	gv := apiflowcontrolv1.SchemeGroupVersion
