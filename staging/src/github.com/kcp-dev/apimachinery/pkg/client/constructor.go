...
type Cache[R any] interface {
	...
	// path so future Cluster / ClusterOrDie calls hand back freshly-built
	// clients without re-caching. The cluster-scoped clientsets in
	// github.com/kcp-dev/client-go forward Evict to their underlying caches,
	// so callers typically invoke Evict on the top-level clientset (which
	// fans out to every sub-client) rather than directly here.
	//
	// Safe to call concurrently with Cluster / ClusterOrDie. No-op if the
